...
def LoadAnalysis(EngineLoad, EngineRPM, VehicleSpeed, TripTime):
    ...
    for i in EngineRPM.index:
        ...
        # Checking whether engine load and engine rpm are less than threshold
...
"""Function checks the coolant temperature and compare it with the threshold.
 Returns Safestate or not."""
...
def Coolant(CoolantTemperatureC, EngineLoad, TripTime):
    ...
    NormalTemperatureC = 190.0
    # Reference form internet safe state 190F to 220F
    ...
    for i in range(0, CoolantTemperatureF_np.size):
        ...
        elif CoolantTemperatureF_np[i] < NormalTemperatureC and \
                CoolantTemperatureF_np[i] > LowestTemperatureC:
            Safestate = Safestate + 1
