"""Function checks the load on the engine and
    returns it as overloaded or not."""
...
def LoadAnalysis(EngineLoad, EngineRPM, VehicleSpeed, TripTime):
    ...
    for i in EngineRPM.index:
        ...
        if EngineLoad[i] > LoadThreshold and EngineRPM[i] > RPMThreshold:
            TempCounterOverload = TempCounterOverload + 1
        CounterOverload.append(TempCounterOverload)

        VehicleSpeed[i] = float(VehicleSpeed[i])

        # ACTUAL SPEED = (ENGINE RPM * PERIMETER OF TYRE)/
        # (AXLE RATIO * GEAR RATIO)
        ExpectedSpeed.append(0.4 * (EngineRPM[i] * 60 * 3.14 * 2 *
                                    TyreSize * 25.4 * 0.000001) / (GearRatio * AxleRatio))
    ...
    return (EngineLoadLess, EngineLoadMore, EngineRPMLess, EngineRPMMore,
            ...
            RPMThreshold, CounterOverload)


"""Function checks the coolant temperature and compare 
it with the threshold. Returns Safestate or not."""
...
def Coolant(CoolantTemperatureC, EngineLoad, TripTime):
    ...
    for i in range(0, CoolantTemperatureF_np.size):
        if (CoolantTemperatureF_np[i] > NormalTemperatureC and
                CoolantTemperatureF_np[i] < HighestTemperatureC):
        ...
        elif (CoolantTemperatureF_np[i] < NormalTemperatureC
              and CoolantTemperatureF_np[i] > LowestTemperatureC):
            Safestate = Safestate + 1
    ...
    return (CoolantTemperatureF_np, HighestTemperatureC, NormalTemperatureC,
            LowestTemperatureC, SafeState1, SafeState2, SafeState3, State0,
            IndexTripTimeThreshold, EngineLoadThreshold
