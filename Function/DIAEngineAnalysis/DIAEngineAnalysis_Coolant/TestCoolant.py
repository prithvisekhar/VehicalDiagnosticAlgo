...
DIAEngineAnalysis = SourceFileLoader("Coolant",
    ...
    def testCoolantTemperature(self):
        try:
            ...
            for i in df_File.index:
                ...
                CoolantTemperatureF_np, HighestTemperatureC, \
                NormalTemperatureC, LowestTemperatureC, SafeState1, \
                SafeState2, SafeState3, State0, IndexTripTimeThreshold, \
                EngineLoadThreshold = DIAEngineAnalysis.Coolant(
                    df['Engine Coolant Temperature(°C)'].replace("-", "0"),
                    df['Engine Load(%)'].replace(to_replace="-", value="0"),
                    df['Trip Time(Since journey start)(s)'].replace("-", "0"))

                TempEngineLoad = df['Engine Load(%)'].replace("-", value="0")
                ...
                axs[0].plot(SafeState1['Index'],
                            CoolantTemperatureF_np[SafeState1['Index']], 'g.')
                axs[0].plot(SafeState2['Index'],
                            CoolantTemperatureF_np[SafeState2['Index']], 'r.')
                axs[0].plot(SafeState3['Index'],
                            CoolantTemperatureF_np[SafeState3['Index']], 'k.')
                ...
                axs[2].plot(np.repeat(EngineLoadThreshold,
                                      len(TempEngineLoad_np)))
                axs[2].plot(SafeState1['Index'],
                            TempEngineLoad_np[SafeState1['Index']], 'g.')
