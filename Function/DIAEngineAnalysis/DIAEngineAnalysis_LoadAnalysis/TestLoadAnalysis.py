		...
		try:
			...
			for i in df_File.index:
				...
				if not os.path.isdir(path):
					os.mkdir(path)
				EngineLoadLess, EngineLoadMore, EngineRPMLess, EngineRPMMore, VehicleSpeedLess, VehicleSpeedMore, ExpectedSpeed,LoadThreshold,RPMThreshold,CounterOverload = DIAEngineAnalysis.LoadAnalysis(df["Engine Load(%)"].replace(to_replace="-", value="0"), df["Engine RPM(rpm)"].replace(to_replace="-", value="0"),df['Speed (GPS)(km/h)'].replace(to_replace="-", value="0"),df['Trip Time(Since journey start)(s)'])
				...
				plt.plot(np.repeat(LoadThreshold,len(TempEngineLoad1)),label='Threshold = '+str(round(LoadThreshold)))
