...
DIASpeedVoilation = SourceFileLoader("SpeedVoilation",
                                     "../Function/DIASpeedVoilation_SpeedVoilation/DIASpeedVoilation.py").load_module()
...
EngineLoadLess, EngineLoadMore, EngineRPMLess, EngineRPMMore, VehicleSpeedLess, VehicleSpeedMore, ExpectedSpeed, LoadThreshold, RPMThreshold, CounterOverload = DIAEngineAnalysis.LoadAnalysis(
    df["Engine Load(%)"], df["Engine RPM(rpm)"], df['Speed (GPS)(km/h)'], df['Trip Time(Since journey start)(s)'])
...
CoolantTemperatureF_np, HighestTemperatureC, NormalTemperatureC, LowestTemperatureC, SafeState1, SafeState2, SafeState3, State0, IndexTripTimeThreshold, EngineLoadThreshold \
    = DIAEngineAnalysis.Coolant(df['Engine Coolant Temperature(°C)'].replace(to_replace="-", value="0"),
